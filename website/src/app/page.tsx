        ...
        <ul className="mx-auto mt-12 flex max-w-3xl flex-wrap justify-center gap-x-8 gap-y-3 text-sm text-charcoal-700">
          {trustBadges.map((badge) => (
            <li key={badge} className="flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-blush-500" aria-hidden />
              {badge}
            </li>
          ))}
        </ul>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {featured.map((t) => (
              <Link
                ...
                className="rounded-2xl border border-blush-100 p-6 transition hover:border-blush-300 hover:shadow-sm"
              >
                ...
                <p className="mt-3 text-sm font-medium text-blush-600">Enquire about this →</p>
              </Link>
            ))}
          </div>
        </section>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2">
            {faqTeasers.map((f) => (
              <div key={f.question} className="rounded-2xl bg-cream-50 p-6">
                <h3 className="font-serif text-base text-blush-700">{f.question}</h3>
                <p className="mt-2 text-sm text-charcoal-700">{f.answer}</p>
              </div>
            ))}
          </div>
          <div className="mt-8 text-center">
            <Link href="/faq" className="text-sm font-medium text-blush-600 hover:text-blush-700">
              See all FAQs →
            </Link>
          </div>
        </div>
