...
- **The WhatsApp AI auto-reply sends customer name/phone/message text to a third-party LLM API (Anthropic)** to generate each reply (`src/common/providers/ai.provider.ts`) — this is a real data flow to a subprocessor, not a local/self-hosted model. It's opt-in (`ANTHROPIC_API_KEY` unset = disabled, every message escalates to staff instead), scoped to Instagram-originated leads only, and never sees Patient records (only pre-conversion Lead data) — but factor it into your privacy policy and DPDP Act/subprocessor-disclosure obligations before enabling it in production. It also never discloses pricing and always escalates anything medical/risk-related rather than answering (see README), but like any LLM-generated output it isn't guaranteed accurate — review a sample of its replies periodically via the CommunicationLog.
